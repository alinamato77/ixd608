		...
		document.querySelectorAll('.pet-fav-btn[data-fav]').forEach(btn => {
     ...
     function initHotdog(toggleId, panelId, tagsId, inputId, clearId) {
	const toggle = document.getElementById(toggleId);
	const panel  = document.getElementById(panelId);
	const tags   = tagsId  ? document.getElementById(tagsId)  : null;
	const input  = inputId ? document.getElementById(inputId) : null;
	const clear  = clearId ? document.getElementById(clearId) : null;
	if (!toggle || !panel) return;

	toggle.addEventListener('click', () => {
		const isOpen = panel.classList.toggle('is-open');
		toggle.setAttribute('aria-expanded', isOpen);
		if (tags) tags.classList.toggle('is-open', isOpen);
